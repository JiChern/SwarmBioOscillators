...
from agent.networks import Policy  # Import SIES architecture (Policy is a GNN model)
from env_test import CDSEnv  # Import CPG environment for oscillatory dynamics
...
class SIESCPG(object):
    """
    A class implementing a Graph-based Central Pattern Generator (SIESCPG) for gait generation.
    ...
    def __init__(self, network, checkpoint, env) -> None:
        ...
        Args:
            ...
            env (CDSEnv): CDS environment instance for state updates.
...
if __name__ == '__main__':
    """
    Main execution block for running the SIES-CPG system on a remote PC.
    ...
    # Initialize SIES-CPG system (8 attention heads)
    heads = 8  
    fd = 64    
    cell_num = 6  
    ...
    env = CDSEnv(cell_nums=cell_num, env_length=500, hz=50) 
    s_cpg = SIESCPG(network=model, checkpoint=checkpoint, env=env)  
    ...
    env.z_mat[0, 0] = 0.1  
    obs = env.z_mat.ravel()  
    rl_encoding = s_cpg.encoding_angle(s_cpg.desired_lag) 
    ...
    all_gait_pub = rospy.Publisher('cpg_gait_all', Float32MultiArray, queue_size=10)  # Publish all phases
    gait_pub = rospy.Publisher('cpg_gait', Float32MultiArray, queue_size=10)  # Publish active cell phases
    ...
    while not rospy.is_shutdown():
        ...
        print('operating legs: ', s_cpg.o_leg_list)
        print('desired phase lags: ', s_cpg.desired_lag)


        # Compute next state and phase angles
        next_state, phase = s_cpg.get_phase_data(state)

        # Publish phases for active cells
        activated_cells = np.array(s_cpg.o_leg_list, dtype=int)
