...
class GATConv(MessagePassing):
    ...
    def forward(  # noqa: F811
        ...
        # Manual replacement for propagate
        if not isinstance(edge_index, Tensor):
            raise NotImplementedError("Manual propagation currently assumes edge_index is a Tensor")
        
        row, col = edge_index[0], edge_index[1]
        x_src, x_dst = x
        x_j = x_src[row]  # [E, H, C]
        msg = self.message(x_j=x_j, alpha=alpha)  # [E, H, C]

        # Determine num_nodes for the output
        num_nodes = size[0] if size is not None else (x_dst.size(0) if x_dst is not None else x_src.size(0))
        out = torch.zeros((num_nodes, H, C), device=x_src.device, dtype=x_src.dtype)
        out = out.index_add(dim=0, index=col, source=msg)  # Aggregate messages to targets
