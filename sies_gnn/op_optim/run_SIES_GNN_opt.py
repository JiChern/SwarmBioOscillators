...
from data_handling import get_data_1
from models import *
import numpy as np
from utils import *
import argparse
...
def train_GNN(model, data, dataset, lr, weight_decay):
    ...
    if dataset == 'Questions':
        ...
        print('lf is bce for questions')
    else:
        lf = nn.CrossEntropyLoss()
        print('lf is ce for other datasets')
...
def objective(trial):
    ...
    # ================== HP Sampling ==================
    if dataset in ['Amazon-ratings']:
        nhid = trial.suggest_categorical('nhid', [128, 256])
    elif dataset in ['Minesweeper', 'roman-empire']:
        nhid = trial.suggest_categorical('nhid', [128, 64])
    else:
    ...
    for split in range(n_splits):
        set_seed(42)
        try:
            print(f"\n=== Trial {trial.number} Split {split} START ===")
            print(f"Before anything     | Allocated: {torch.cuda.memory_allocated()/1e9:.2f} GB | Reserved: {torch.cuda.memory_reserved()/1e9:.2f} GB")

            data = get_data_1(data_dir, dataset, split).to(device)
            print(f"✅ data.to(device)  | Allocated: {torch.cuda.memory_allocated()/1e9:.2f} GB")
            ...
            trial.report(np.mean(best), split + 1)
...
def optimize(storage, study_name, n_trials_per_process):
    ...
    study.optimize(objective, n_trials=n_trials_per_process, gc_after_trial=True)
