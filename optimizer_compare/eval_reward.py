...
def get_eval_reward(cell_num, edge_index, model, env):
    """
    ...
    """
    # Define evaluating target phase lag patterns (4 different patterns for 8 cells)
    env.desired_lag_list = np.array([[0,0,0,0,0,0,0,0],          
...
if __name__ == '__main__':
    ...
    cwd = os.getcwd()
    csvfile = open(cwd + '/data/train_reward_GCPG.csv', 'w')
    ...
    for i in np.arange(5,305,5):
        ...
        reward_sum_list = get_eval_reward(cell_num=cell_num, edge_index=ei, model=model, env=env)
