...
class CPGEnv(object):
    ...
    def encoding_angle_batch(self, desired_lag_list):
        """Encode desired phase lags using trigonometric functions.
    
        Args:
            desired_lag_list (torch.Tensor): Desired phase lags. Supports batched input [batch_size, cell_num].
    
        Returns:
            torch.Tensor: Encoding matrix (shape: [batch_size, cell_num, 2] for batched, or [cell_num, 2] for single).
        """
        if desired_lag_list.dim() == 1:
            desired_lag_list = desired_lag_list.unsqueeze(0)
        batch_size = desired_lag_list.shape[0]
        encoding_mat = torch.zeros((batch_size, self.cell_nums, 2), device=self.device)
        angles = desired_lag_list * 2 * torch.pi  # Use torch.pi for device compatibility
        encoding_mat[..., 0] = torch.sin(angles)
        encoding_mat[..., 1] = torch.cos(angles)
        if batch_size == 1:
            encoding_mat = encoding_mat.squeeze(0)
        return encoding_mat


    def hopf_batch(self, x, y, ax, ay):
        """Compute the derivatives for Hopf oscillator dynamics.
    
        Args:
            x, y, ax, ay (torch.Tensor): Positions and actions. Supports batched [batch_size, cell_num] or single [cell_num].
    
        Returns:
            tuple[torch.Tensor, torch.Tensor]: Derivatives (same shape as x/y).
        """
        alpha = 10
        beta = 10
        if x.dim() == 1:
            ...
            mu = torch.ones((batch_size, self.cell_nums), device=self.device)
        r_2 = x * x + y * y
        dx = alpha * (mu - r_2) * x - self.omega * y + ax
        dy = beta * (mu - r_2) * y + self.omega * x + ay
        return dx, dy

    def step_env_batch(self, action):
        """Execute one step of the environment with given actions.
    
        Args:
            action (torch.Tensor): Actions for oscillators. Supports batched input [batch_size, cell_num, 2] or single [cell_num * 2].
    
        Returns:
            torch.Tensor: Next observation (batched [batch_size, cell_num*4] or single [cell_num*4]).
        """
        if action.dim() == 1:
            # Single instance
            batch_size = 1
        ...
        dx, dy = self.hopf_batch(x, y, action_x, action_y)
        ...
        if batch_size == 1:
            ...
            self.z_mat[:, :, 0] = x + dx * self.dt
            self.z_mat[:, :, 1] = y + dy * self.dt
            obs = self.z_mat.view(batch_size, -1)
        
        rl_encoding = self.encoding_angle_batch(self.desired_lag)
        
        if batch_size == 1:
            obs = torch.cat((obs, rl_encoding.view(-1)))
        else:
            obs = torch.cat((obs, rl_encoding.view(batch_size, -1)), dim=1)
        
        self.internal_step += 1  # Scalar increment; assumes shared across batch (or make vector if per-instance)
        return obs
