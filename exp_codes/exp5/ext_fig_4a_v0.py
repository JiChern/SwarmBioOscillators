...
from agent.networks import PolicyStateSpace # import V0 architecture
...
def get_train_error(cell_num, edge_index, model, env):
    ...
    Args:
        ...
        model (V0): Trained GNN policy model.
...
if __name__ == '__main__':
    ...
    """
    # Set-up the V0 model, with 8 attention heads
    heads = 1
    fd = 512

    # Initialize PolicyStateSpace model with specified heads and feature dimension
    model = PolicyStateSpace(heads=heads, feature_dim=fd)
    ...
    cwd = os.getcwd()
    csvfile = open(cwd + '/data/train_error_V0.csv', 'w')
    ...
    for i in np.arange(5, 305, 5):
        ...
        checkpoint = torch.load(cwd + '/checkpoints/V0/model-' + str(i) + '0000-CPG_r_i.pt', weights_only=True) 
